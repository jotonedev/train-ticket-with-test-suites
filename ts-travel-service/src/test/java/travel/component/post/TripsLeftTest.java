...
public class TripsLeftTest {
     ...
     * <li><b>Related Issue:</b></li> <b>S1:</b> when there is no route that matches the trip, route should be null.
     * but will be configured to be an empty route by the travel service, which leads to a NullPointerException when
     * a field of the route is accessed. As the fallback response is triggered with a 200 OK, it remains unclear whether
    ...
    public void testQueryDepartureTimeFromPast() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/travelservice/trips/left")
                ...
                .andExpect(jsonPath("$.msg", is("No Trip info content")))
                .andExpect(jsonPath("$.data", nullValue()));
    }
