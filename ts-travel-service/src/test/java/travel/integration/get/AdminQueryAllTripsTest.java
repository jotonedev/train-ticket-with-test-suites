...
public class AdminQueryAllTripsTest {
    ...
    public void testAdminQueryAllValidObjects() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/travelservice/admin_trip"))
                ...
                .andExpect(jsonPath("$.data[0].route.id", is("92708982-77af-4318-be25-57ccb0ff69ad")))
                // check that trip of array element matches initData
                .andExpect(jsonPath("$.data[0].trip.trainTypeId", is("GaoTieOne")))
                .andExpect(jsonPath("$.data[0].trip.tripId.type", is("G")))
                .andExpect(jsonPath("$.data[0].trip.tripId.number", is("1234")))
                .andExpect(jsonPath("$.data[0].trip.startingStationId", is("shanghai")))
                .andExpect(jsonPath("$.data[0].trip.terminalStationId", is("taiyuan")))
                .andExpect(jsonPath("$.data[0].trip.startingTime", is("2013-05-04T01:00:00.000+00:00")))
