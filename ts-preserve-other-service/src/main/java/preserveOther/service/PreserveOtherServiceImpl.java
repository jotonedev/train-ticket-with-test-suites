...
public class PreserveOtherServiceImpl implements PreserveOtherService
{
    ...
    public Response preserve(OrderTicketsInfo oti, HttpHeaders httpHeaders)
    {
        ...
        if (response.getStatus() == 0) {
            PreserveOtherServiceImpl.LOGGER.info("[Preserve Service][Search For Trip Detail Information] {}",
                response.getMsg());
        ...
        PreserveOtherServiceImpl.LOGGER.info("[Preserve Other Service][Order] Order Travel Date: {}",
            oti.getDate().toString());
        ...
        if (cor.getStatus() == 0) {
            PreserveOtherServiceImpl.LOGGER.info(
                "[Preserve Other Service][Create Order Fail] Create Order Fail.  Reason: {}", cor.getMsg());
