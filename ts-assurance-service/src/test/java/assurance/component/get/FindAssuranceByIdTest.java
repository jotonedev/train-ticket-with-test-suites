...
@ActiveProfiles("test")
public class FindAssuranceByIdTest {
     ...
     * <li><b>Parameters:</b></li> an existing assuranceId
     * <li><b>Expected result:</b></li> status 1, msg "Find Assurace Success", the found Assurance object.
     * </ul>
     * @throws Exception
     */
    @Test
    public void testFindAssuranceByIdCorrectObject() throws Exception {
        // Arrange
        Assurance assurance = configureAssurance(UUID.randomUUID(), UUID.randomUUID());
        assuranceRepository.save(assurance);

        // Act
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/assuranceservice/assurances/assuranceid/{assuranceId}", assurance.getId())
     ...
     */
    @Test
    public void testFindAssuranceByIdMultipleId() throws Exception {
        // Arrange
        Assurance assurance = configureAssurance(UUID.randomUUID(), UUID.randomUUID());
        assuranceRepository.save(assurance);

        // Act
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/assuranceservice/assurances/assuranceid/{assuranceId}", assurance.getId(), UUID.randomUUID())
                        .contentType(MediaType.APPLICATION_JSON))
                // Assert
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(1)))
     ...
     */
    @Test
    public void testFindAssuranceByIdNotExists() throws Exception {
        // Arrange
        UUID id = UUID.randomUUID();

        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/assuranceservice/assurances/assuranceid/{assuranceId}", id)
                        .contentType(MediaType.APPLICATION_JSON))
                // Assert
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(0)))
                .andExpect(jsonPath("$.msg", is("No Conotent by this id")))
     ...
     */
    @Test
    public void testFindAssuranceByIdMissingId() {
        // Act & Assert
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/assuranceservice/assurances/assuranceid/{assuranceId}"));
        });
    }
     ...
     */
    @Test
    public void testFindAssuranceByIdMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/assuranceservice/assurances/assuranceid/{assuranceId}",UUID.randomUUID() + "/" + UUID.randomUUID()))
                // Assert
     ...
     */
    @Test
    public void FAILING_testFindAssuranceByIdInvalidUUIDFormat() throws Exception {
        // Arrange
        String uuid = "Does not follow UUID format";

        // Act
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/assuranceservice/assurances/assuranceid/{assuranceId}", uuid)
                        .contentType(MediaType.APPLICATION_JSON))
                // Assert
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(0)))
