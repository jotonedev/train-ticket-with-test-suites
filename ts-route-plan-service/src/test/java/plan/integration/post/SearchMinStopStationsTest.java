...
public class SearchMinStopStationsTest {
    ...
    /**
     * There is an issue with the initialization of the containers, as the travel services are not
     * executing when sending the initial requests, resulting in a fallback response due to exceeding the timeout.
     * Since this issue is not related to the implementation of the services, we work around it by sending the initial
     * request manually and ignoring the response. All subsequent requests are then executed as expected.
     */
    @BeforeEach
    void sendInitialRequest() {
        TripAllDetailInfo tripAllDetailInfo = new TripAllDetailInfo();
        tripAllDetailInfo.setTripId("Z1235");
        tripAllDetailInfo.setFrom("Nan Jing");
        tripAllDetailInfo.setTo("Bei Jing");
        tripAllDetailInfo.setTravelDate(new Date("Sat Jul 29 00:00:00 GMT+0800 3025"));
        HttpEntity requestEntity = new HttpEntity(tripAllDetailInfo);

        restTemplate.exchange(
                "http://" + tsTravel2ServiceUrl + ":" + tsTravel2ServicePort + "/api/v1/travel2service/trip_detail",
                HttpMethod.POST,
                requestEntity,
     ...
     * <li><b>Related Issue:</b></li> <b>S1:</b> When the station names are configured such that the route service
     * is unable to find routes with the startId and terminalId, the data of the response is null, which will trigger
     * a NullPointerException when trying to access the size of the data. As the fallback response is triggered with a
     * 200 OK, it remains unclear whether developers deliberately omitted preventative measures (e.g. null-checks)
     * in favor of relying on the fallback (which would be considered intended behavior), or not.
     * </ul>
     * @throws Exception
     */
    @Test
    @Order(1)
    public void testSearchMinStopStationsNoTrips() throws Exception {
        // Arrange
    ...
    @Test
    @Order(3)
    public void testSearchMinStopStationsServiceUnavailable() throws Exception {
        // Arrange
