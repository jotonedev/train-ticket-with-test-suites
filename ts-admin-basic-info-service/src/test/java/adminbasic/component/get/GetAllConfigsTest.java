...
@ActiveProfiles("test")
public class GetAllConfigsTest {
     ...
     */
    @Test
    public void testGetAllConfigsConfigServiceSuccessful() throws Exception {
        // Arrange
        List<Config> configList = new ArrayList<>();
        Config expectedConfig = generateRandomConfig();
        configList.add(expectedConfig);
        int expectedConfigListLength = configList.size();
        Response<List<Config>> configResponse = new Response<>(1, "Find all  config success", configList);
        ...
        // Act
        mockMvc.perform(get("/api/v1/adminbasicservice/adminbasic/configs"))
                .andDo(print())
                // Assert
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(1)))
                .andExpect(jsonPath("$.msg", is("Find all  config success")))
                .andExpect(jsonPath("$.data").isArray())
                .andExpect(jsonPath("$.data", hasSize(expectedConfigListLength)))
                .andExpect(jsonPath("$.data[0].name", is(expectedConfig.getName())))
                .andExpect(jsonPath("$.data[0].description", is(expectedConfig.getDescription())));
    }
     ...
     */
    @Test
    public void testGetAllConfigsConfigServiceCrash() throws Exception {
        // Arrange
        when(restTemplate.exchange(
                ArgumentMatchers.contains("http://ts-config-service:15679/api/v1/configservice/configs"),
                ArgumentMatchers.eq(HttpMethod.GET),
