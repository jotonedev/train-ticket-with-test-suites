...
public class GetAllPricesTest {
     ...
     */
    @Test
    public void testGetAllPricesPriceServiceSuccessful() throws Exception {
        // Arrange
        List<PriceInfo> priceInfoList = new ArrayList<>();
        PriceInfo expectedPriceInfo = generateRandomPriceInfo();
        priceInfoList.add(expectedPriceInfo);
        int expectedPriceInfoListLength = priceInfoList.size();
        Response<List<PriceInfo>> priceResponse = new Response<>(1, "Success", priceInfoList);
        ...
        // Act
        mockMvc.perform(get("/api/v1/adminbasicservice/adminbasic/prices"))
                .andDo(print())
                // Assert
                ...
                .andExpect(jsonPath("$.data", hasSize(expectedPriceInfoListLength)))
                .andExpect(jsonPath("$.data[0].id", is(expectedPriceInfo.getId())));
    }
     ...
     */
    @Test
    public void testGetAllPricesPriceServiceCrash() throws Exception {
        // Arrange
        when(restTemplate.exchange(
                ArgumentMatchers.contains("http://ts-price-service:16579/api/v1/priceservice/prices"),
                ArgumentMatchers.eq(HttpMethod.GET),
