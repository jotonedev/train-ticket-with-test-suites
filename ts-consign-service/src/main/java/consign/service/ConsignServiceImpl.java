...
public class ConsignServiceImpl implements ConsignService
{
    ...
    @Value("${ts.consign.price.service.url:ts-consign-price-service}")
    private String tsConsignPriceServiceUrl;

    @Value("${ts.consign.price.service.port:16110}")
    private String tsConsignPriceServicePort;
    ...
    public Response insertConsignRecord(Consign consignRequest, HttpHeaders headers)
    {
        ...
        ResponseEntity<Response<Double>> re = restTemplate.exchange(
            "http://"+tsConsignPriceServiceUrl+":"+tsConsignPriceServicePort+"/api/v1/consignpriceservice/consignprice/"
                + consignRequest.getWeight() + "/" + consignRequest.isWithin(),
            HttpMethod.GET,
            requestEntity,
