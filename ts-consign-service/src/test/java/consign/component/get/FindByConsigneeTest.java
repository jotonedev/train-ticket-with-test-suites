...
@ActiveProfiles("test")
public class FindByConsigneeTest {
    ...
    /**
     * <ul>
     * <li><b>Tests:</b></li> how the endpoint behaves when we provide an existing consignee as the URL parameter
     * <li><b>Parameters:</b></li> a consignee that exists in the database as a ConsignRecord
     * <li><b>Expected result:</b></li> status 1, msg "Find consign by consignee success", the list of ConsignRecords that match the consignee
     * </ul>
     * @throws Exception
     */
    @Test
    public void testFindByConsigneeCorrectObject() throws Exception {
        ...
        // Act
        mockMvc.perform(get("/api/v1/consignservice/consigns/{consignee}", record.getConsignee())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(1)))
                .andExpect(jsonPath("$.msg", is("Find consign by consignee success")))
    ...
    /**
     * <ul>
     * <li><b>Tests:</b></li> how the endpoint behaves when we provide a consignee that does not exist in the database
     * <li><b>Parameters:</b></li> a random consignee that does not exist in the database
    ...
    public void testFindByConsigneeZeroObjects() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/consignservice/consigns/{consignee}", UUID.randomUUID())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Arrange
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(0)))
                .andExpect(jsonPath("$.msg", is("No Content according to consignee")))
                .andExpect(jsonPath("$.data", is(nullValue())));
    }

    /**
     * <ul>
     * <li><b>Tests:</b></li> how the endpoint behaves when we give the endpoint request more than one consignee as the URL parameter
     * <li><b>Parameters:</b></li> an element that matches the consignee given in paths and additional random strings
     * <li><b>Expected result:</b></li> status 1, msg "Find consign by consignee success", the list of ConsignRecords that match the consignee.
    ...
    public void testFindByConsigneeMultipleId() throws Exception {
        ...
        // Act
        mockMvc.perform(get("/api/v1/consignservice/consigns/{consignee}", record.getConsignee(), UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(1)))
    ...
    public void testFindByConsigneeMissingId() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/consignservice/consigns/{consignee}"));
        });
    }
    ...
    public void testFindByConsigneeMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/consignservice/consigns/{consignee}",UUID.randomUUID() + "/" + UUID.randomUUID()))
                // Assert
