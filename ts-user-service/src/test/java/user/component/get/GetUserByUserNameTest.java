...
public class GetUserByUserNameTest {
    ...
    public void testGetUserByUserNameNotExist() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/userservice/users/{userName}", UUID.randomUUID()))
                .andDo(print())
                // Assert
     ...
     */
    @Test
    public void testGetUserByUserNameElementInDatabase() throws Exception {
        // Arrange
        User user = populateDatabase();

        // Act
        mockMvc.perform(get("/api/v1/userservice/users/{userName}", user.getUserName()))
                .andDo(print())
                // Assert
                .andExpect(status().isOk())
     ...
     */
    @Test
    public void testGetUserByUserNameMultipleId() throws Exception {
        // Arrange
        User user = populateDatabase();

        // Act
        mockMvc.perform(get("/api/v1/userservice/users/{userName}", user.getUserName(), UUID.randomUUID().toString()))
                .andDo(print())
                // Assert
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(1)))
     ...
     */
    @Test
    public void testGetUserByUserNameMissingId() {
        // Act & Assert
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/userservice/users/{userName}"));
        });
    }
     ...
     */
    @Test
    public void testGetUserByUserNameMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/userservice/users/{userName}",UUID.randomUUID().toString() + "/" + UUID.randomUUID().toString()))
                // Assert
