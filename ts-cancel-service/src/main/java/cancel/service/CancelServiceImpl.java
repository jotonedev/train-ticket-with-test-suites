...
public class CancelServiceImpl implements CancelService
{
        ...
        if (orderResult.getStatus() == 1) {
                ...
                || order.getStatus() == OrderStatus.PAID.getCode() || order.getStatus() == OrderStatus.CHANGE.getCode())
            {
                ...
                if (changeOrderResult.getStatus() == 1) {
                    ...
                    CancelServiceImpl.LOGGER.info("[Cancel Order Service][Cancel Order] Fail.Reason: {}",
    ...
    public boolean sendEmail(NotifyInfo notifyInfo, HttpHeaders headers)
    {
        ...
        ResponseEntity<Boolean> re = restTemplate.exchange(
            "http://"+tsNotificationServiceUrl+":"+tsNotificationServicePort+"/api/v1/notifyservice/notification/order_cancel_success",
            HttpMethod.POST,
            requestEntity,
            Boolean.class);
        return re.getBody();
                    ...
                    || order.getStatus() == OrderStatus.PAID.getCode())
                {
                    if (order.getStatus() == OrderStatus.NOTPAID.getCode()) {
                        CancelServiceImpl.LOGGER.info(
                            "[Cancel Order][Refund Price] From Order Other Service.Not Paid.");
    ...
    public Response<User> getAccount(String orderId, HttpHeaders headers)
    {
        ...
        ResponseEntity<Response<User>> re = restTemplate.exchange(
            "http://"+tsUserServiceUrl+":"+tsUserServicePort+"/api/v1/userservice/users/id/" + orderId,
            HttpMethod.GET,
            requestEntity,
    ...
    private Response cancelFromOtherOrder(Order info, HttpHeaders headers)
    {
        ...
        ResponseEntity<Response> re = restTemplate.exchange(
            "http://"+tsOrderOtherServiceUrl+":"+tsOrderOtherServicePort+"/api/v1/orderOtherService/orderOther",
            HttpMethod.PUT,
            requestEntity,
            Response.class);
