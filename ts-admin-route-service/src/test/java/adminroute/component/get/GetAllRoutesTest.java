package adminroute.component.get;
...
public class GetAllRoutesTest {
     ...
     */
    @Test
    public void testGetAllRoutesRouteServiceSuccessful() throws Exception {
        // Arrange
        ArrayList<Route> routesList = new ArrayList<>();
        Route expectedRoute = generateRandomRoute();
        routesList.add(expectedRoute);
        int expectedRoutesListLength = routesList.size();
        Response<ArrayList<Route>> routeResponse = new Response<>(1, "Success", routesList);
        ...
        mockMvc.perform(get("/api/v1/adminrouteservice/adminroute"))
                ...
                .andExpect(jsonPath("$.data", hasSize(expectedRoutesListLength)))
                .andExpect(jsonPath("$.data[0].id", is(expectedRoute.getId())))
                .andExpect(jsonPath("$.data[0].startStationId", is(expectedRoute.getStartStationId())))
                .andExpect(jsonPath("$.data[0].terminalStationId", is(expectedRoute.getTerminalStationId())));
    }
     ...
     */
    @Test
    public void testGetAllRoutesRouteServiceCrash() throws Exception {
        // Arrange
        when(restTemplate.exchange(
                ArgumentMatchers.contains("http://ts-route-service:11178/api/v1/routeservice/routes"),
                ArgumentMatchers.eq(HttpMethod.GET),
