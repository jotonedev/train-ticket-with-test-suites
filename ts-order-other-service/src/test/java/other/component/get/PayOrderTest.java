...
public class PayOrderTest {
    ...
    public void testPayOrderMatchingId() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderOtherService/orderOther/orderPay/{orderId}", order.getId().toString())
    ...
    public void testPayOrderMultipleId() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderOtherService/orderOther/orderPay/{orderId}", order.getId().toString(), UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
    ...
    public void testPayOrderNonExistingId() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/orderOtherService/orderOther/orderPay/{orderId}"));
        });
    }
    ...
    public void FAILING_testPayOrderIncorrectFormatId() throws Exception {
        ...
        // Act
        mockMvc.perform(get("/api/v1/orderOtherService/orderOther/orderPay/{orderId}", uuid)
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
    ...
    public void testPayOrderMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/orderOtherService/orderOther/orderPay/{orderId}",UUID.randomUUID().toString() + "/" + UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
