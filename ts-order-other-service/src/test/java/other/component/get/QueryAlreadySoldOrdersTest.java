...
public class QueryAlreadySoldOrdersTest {
    ...
    public void FIXED_PREVIOUSLY_FAILING_testQueryAlreadySoldOrdersMatchingOrder() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderOtherService/orderOther/{travelDate}/{trainNumber}", order.getTravelDate().toString(), order.getTrainNumber())
    ...
    public void FIXED_PREVIOUSLY_FAILING_testQueryAlreadySoldOrdersMultipleData() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderOtherService/orderOther/{travelDate}/{trainNumber}", order.getTravelDate().toString(), order.getTrainNumber(), new Date(), UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
    ...
    public void testQueryAlreadySoldOrdersNonExistingTrainNumber() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/orderOtherService/orderOther/{travelDate}/{trainNumber}", new Date().toString()));
        });
    }
