...
public class GetOrderPriceTest {
    ...
    public void testGetOrderPriceMatchingId() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderOtherService/orderOther/price/{orderId}", order.getId().toString())
    ...
    public void testGetOrderPriceMultipleId() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderOtherService/orderOther/price/{orderId}", order.getId().toString(), UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                .andExpect(status().isOk())
                // Assert
                .andReturn().getResponse().getContentAsString();
    ...
    public void testGetOrderPriceNonExistingId() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/orderOtherService/orderOther/price/{orderId}"));
        });
    }
    ...
    public void FAILING_testGetOrderPriceIncorrectFormatId() throws Exception {
        ...
        // Act
        mockMvc.perform(get("/api/v1/orderOtherService/orderOther/price/{orderId}", uuid)
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
    ...
    public void testGetOrderPriceMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/orderOtherService/orderOther/price/{orderId}", UUID.randomUUID().toString() + "/" + UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
