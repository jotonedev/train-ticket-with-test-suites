...
@ActiveProfiles("test")
public class GetOrderPriceTest {
     ...
     */
    @Test
    public void testGetOrderPriceMatchingId() throws Exception {
        // Arrange
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/price/{orderId}", order.getId().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        Response<String> response = new ObjectMapper().readValue(result, typeFactory.constructParametricType(Response.class, String.class));
     ...
     * <li><b>Expected result:</b></li> status 1, msg "Order Not Found", data "-1.0"
     * </ul>
     * @throws Exception
     */
    @Test
    public void testGetOrderPriceNotMatchingId() throws Exception {
        // Arrange
        Order order = createSampleOrder();
        TypeFactory typeFactory = new ObjectMapper().getTypeFactory();
     ...
     * <li><b>Expected result:</b></li> status 1, msg "Success.", the specified order. Only the first id in paths is used.
     * </ul>
     * @throws Exception
     */
    @Test
    public void testGetOrderPriceMultipleId() throws Exception {
        // Arrange
        Order order = createSampleOrder();
        orderRepository.save(order);
        TypeFactory typeFactory = new ObjectMapper().getTypeFactory();
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/price/{orderId}", order.getId().toString(), UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                .andExpect(status().isOk())
                // Assert
                .andReturn().getResponse().getContentAsString();
     ...
     */
    @Test
    public void testGetOrderPriceNonExistingId() {
        // Act & Assert
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/orderservice/order/price/{orderId}"));
        });
    }
     ...
     * <li><b>Related Issue:</b></li> <b>F27f:</b> The implementation of the service does not check whether the id is in
     ...
     */
    @Test
    public void FAILING_testGetOrderPriceIncorrectFormatId() throws Exception {
        // Arrange
        String uuid = "not a correct format id";

        // Act
        mockMvc.perform(get("/api/v1/orderservice/order/price/{orderId}", uuid)
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
     ...
     */
    @Test
    public void testGetOrderPriceMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/orderservice/order/price/{orderId}", UUID.randomUUID().toString() + "/" + UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
