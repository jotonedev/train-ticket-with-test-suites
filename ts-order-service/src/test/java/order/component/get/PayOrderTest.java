...
public class PayOrderTest {
     ...
     * <li><b>Expected result:</b></li> status 1, msg "Pay Order Success.", a configured order object
     * </ul>
     * @throws Exception
     */
    @Test
    public void testPayOrderMatchingId() throws Exception {
        // Arrange
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/orderPay/{orderId}", order.getId().toString())
        ...
        Assertions.assertEquals(new Response<>(1, "Pay Order Success.", order), response);
    }
     ...
     */
    @Test
    public void testPayOrderNoMatchingId() throws Exception {
        // Arrange
        Order order = createSampleOrder();
        TypeFactory typeFactory = new ObjectMapper().getTypeFactory();
     ...
     * <li><b>Expected result:</b></li> status 1, msg "Pay Order Success.", a configured order object. Only the first needed values are used.
     * </ul>
     * @throws Exception
     */
    @Test
    public void testPayOrderMultipleId() throws Exception {
        // Arrange
        Order order = createSampleOrder();
        orderRepository.save(order);
        order.setStatus(OrderStatus.PAID.getCode());
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/orderPay/{orderId}", order.getId().toString(), UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
     ...
     */
    @Test
    public void testPayOrderNonExistingId() {
        // Act & Assert
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/orderservice/order/orderPay/{orderId}"));
        });
    }
     ...
     */
    @Test
    public void FAILING_testPayOrderIncorrectFormatId() throws Exception {
        // Arrange
        String uuid = "not a correct format id";

        // Act
        mockMvc.perform(get("/api/v1/orderservice/order/orderPay/{orderId}", uuid)
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
     ...
     */
    @Test
    public void testPayOrderMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/orderservice/order/orderPay/{orderId}",UUID.randomUUID().toString() + "/" + UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
