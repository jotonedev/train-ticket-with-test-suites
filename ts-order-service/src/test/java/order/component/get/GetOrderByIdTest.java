...
public class GetOrderByIdTest {
    ...
    /**
     * <ul>
     * <li><b>Tests:</b></li> how the endpoint behaves when we pass an id that matches the id of an order in the database
     * <li><b>Parameters:</b></li> An order with an id that matches the id of an order in the database
     * <li><b>Expected result:</b></li> status 1, msg "Success.", the specified order
     * </ul>
     * @throws Exception
     */
    @Test
    public void testGetOrderByIdMatchingId() throws Exception {
        // Arrange
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/{orderId}", order.getId().toString())
    ...
    /**
     * <ul>
     * <li><b>Tests:</b></li> how the endpoint behaves when we pass an id that does not exist in the database
     * <li><b>Parameters:</b></li> An order with an id that does not match the id of any order in the database
     * <li><b>Expected result:</b></li> status 0, msg "Order Not Found", no data
     * </ul>
     * @throws Exception
     */
    @Test
    public void testGetOrderByIdNoMatchingId() throws Exception {
        // Arrange
        Order order = createSampleOrder();
        TypeFactory typeFactory = new ObjectMapper().getTypeFactory();
     ...
     */
    @Test
    public void testGetOrderByIdMultipleId() throws Exception {
        // Arrange
        Order order = createSampleOrder();
        orderRepository.save(order);
        TypeFactory typeFactory = new ObjectMapper().getTypeFactory();
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/{orderId}", order.getId().toString(), UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                .andExpect(status().isOk())
                // Assert
                .andReturn().getResponse().getContentAsString();
     ...
     */
    @Test
    public void testGetOrderByIdNonExistingId() {
        // Act & Assert
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/orderservice/order/{orderId}"));
        });
    }
     ...
     */
    @Test
    public void FAILING_testGetOrderByIdIncorrectFormatId() throws Exception {
        // Arrange
        String uuid = "not a correct format id";

        // Act
        mockMvc.perform(get("/api/v1/orderservice/order/{orderId}", uuid)
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
