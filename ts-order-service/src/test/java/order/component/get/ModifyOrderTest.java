...
public class ModifyOrderTest {
    ...
    public void testModifyOrderMatchingId() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/status/{orderId}/{status}", order.getId().toString(), OrderStatus.PAID.getCode())
    ...
    public void testModifyOrderMultipleId() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/status/{orderId}/{status}", order.getId().toString(), OrderStatus.PAID.getCode(), UUID.randomUUID().toString(), OrderStatus.PAID.getCode())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
    ...
    public void testModifyOrderNonExistingId() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/orderservice/order/status/{orderId}/{status}", OrderStatus.PAID.getCode()));
        });
    }
    ...
    public void FAILING_testModifyOrderIncorrectFormatId() throws Exception {
        ...
        // Act
        mockMvc.perform(get("/api/v1/orderservice/order/status/{orderId}/{status}", uuid, OrderStatus.PAID.getCode())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
    ...
    public void testModifyOrderMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/orderservice/order/status/{orderId}/{status}", UUID.randomUUID().toString() + "/" + UUID.randomUUID().toString(), OrderStatus.PAID.getCode())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
