...
public class CheckSecurityAboutOrderTest {
    ...
    public void FIXED_PREVIOUSLY_FAILING_testCheckSecurityAboutOrderMatchingId() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/security/{checkDate}/{accountId}", order.getTravelDate().toString(), order.getAccountId().toString())
        ...
        Assertions.assertEquals(new Response<>(1, "Check Security Success . ", security), response);
    }
    ...
    public void FIXED_PREVIOUSLY_FAILING_testCheckSecurityAboutOrderMultipleId() throws Exception {
        ...
        // Act
        String result = mockMvc.perform(get("/api/v1/orderservice/order/security/{checkDate}/{accountId}", order.getTravelDate().toString(), order.getAccountId().toString(), new Date(), UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
    ...
    public void testCheckSecurityAboutOrderNonExistingId() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/orderservice/order/security/{checkDate}/{accountId}", new Date()));
        });
    }
     ...
     * <b>F9a:</b> Endpoints of this service take a Date object as a parameter in paths. However, since the arguments of
     ...
     * This particular test case is however still failing due to <b>F27f</b>.
     * </ul>
     * @throws Exception
     */
    @Test
    public void FAILING_testCheckSecurityAboutOrderIncorrectFormatId() throws Exception {
        ...
        // Act
        mockMvc.perform(get("/api/v1/orderservice/order/security/{checkDate}/{accountId}", date, uuid)
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
