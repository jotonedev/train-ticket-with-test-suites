...
@ActiveProfiles("test")
public class TicketCollectTest {
     ...
     */
    @Test
    public void testTicketCollectCorrectIdOrderService() throws  Exception {
        // Arrange
        ...
        uri = UriComponentsBuilder.fromUriString("http://ts-order-service:12031/api/v1/orderservice/order/status/" + order.getId() + "/" + OrderStatus.COLLECTED.getCode()).build().toUri();
        ...
        // Act
        mockMvc.perform(get("/api/v1/executeservice/execute/collected/{orderId}", order.getId())
     ...
     */
    @Test
    public void testTicketCollectCorrectIdOrderServiceMultipleId() throws  Exception {
        // Arrange
        Order order = new Order();
        order.setStatus(OrderStatus.PAID.getCode());
        order.setId(UUID.randomUUID());
        ...
        // Act
        mockMvc.perform(get("/api/v1/executeservice/execute/collected/{orderId}", order.getId(), UUID.randomUUID().toString())
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(1)))
     ...
     */
    @Test
    public void testTicketCollectIdNotExists() throws Exception {
        // Arrange
        ...
        // Act
        mockMvc.perform(get("/api/v1/executeservice/execute/collected/{orderId}", orderId)
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
     ...
     */
    @Test
    public void testTicketCollectCorrectIdOrderOtherService() throws Exception {
        // Arrange
        Order order = new Order();
        order.setStatus(OrderStatus.PAID.getCode());
        order.setId(UUID.randomUUID());
        ...
        uri = UriComponentsBuilder.fromUriString("http://ts-order-other-service:12032/api/v1/orderOtherService/orderOther/status/" + order.getId() + "/" + OrderStatus.COLLECTED.getCode()).build().toUri();
     ...
     */
    @Test
    public void testTicketCollectMissingVariable() {
        // Act & Assert
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/executeservice/execute/collected/{orderId}"));
        });
    }
     ...
     */
    @Test
    public void testTicketCollectMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/executeservice/execute/collected/{orderId}",UUID.randomUUID() + "/" + UUID.randomUUID()))
                // Assert
