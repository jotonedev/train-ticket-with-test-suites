...
import inside_payment.entity.Order;
import inside_payment.entity.OrderStatus;
...
public class InsidePaymentServiceImpl implements InsidePaymentService
{
    ...
    @Value("${ts.payment.service.url:ts-payment-service}")
    private String tsPaymentServiceUrl;

    @Value("${ts.payment.service.port:19001}")
    private String tsPaymentServicePort;
    ...
    public Response pay(PaymentInfo info, HttpHeaders headers)
    {
        ...
        if (info.getTripId().startsWith("G") || info.getTripId().startsWith("D")) {
            requestOrderURL = "http://"+tsOrderServiceUrl+":"+tsOrderServicePort+"/api/v1/orderservice/order/" + info.getOrderId();
        } else {
            requestOrderURL =
                "http://"+tsOrderOtherServiceUrl+":"+tsOrderOtherServicePort+"/api/v1/orderOtherService/orderOther/" + info.getOrderId();
        }
        ...
        if (result.getStatus() == 1) {
            ...
            if (order.getStatus() != OrderStatus.NOTPAID.getCode()) {
                InsidePaymentServiceImpl.LOGGER.info(
                    "[Inside Payment Service][Pay] Error. Order status Not allowed to Pay.");
            ...
            if (totalExpand.compareTo(money) > 0) {
                ...
                ResponseEntity<Response> reOutsidePaySuccess = restTemplate.exchange(
                    "http://"+tsPaymentServiceUrl+":"+tsPaymentServicePort+"/api/v1/paymentservice/payment",
                    HttpMethod.POST,
    ...
    public void initPayment(Payment payment, HttpHeaders headers)
    {
        ...
        if (!paymentTemp.isPresent()) {
            ...
            InsidePaymentServiceImpl.LOGGER.info("[Inside Payment Service][Init Payment] Already Exists: {}",
                payment.getId());
        }
    }
    ...
    private Response setOrderStatus(String tripId, String orderId, HttpHeaders headers)
    {
        ...
        if (tripId.startsWith("G") || tripId.startsWith("D")) {
            ...
            ResponseEntity<Response> reModifyOrderStatusResult = restTemplate.exchange(
                "http://"+tsOrderServiceUrl+":"+tsOrderServicePort+"/api/v1/orderservice/order/status/" + orderId + "/" + orderStatus,
                HttpMethod.GET,
            ...
            HttpEntity requestEntityModifyOrderStatusResult = new HttpEntity(headers);
            ResponseEntity<Response> reModifyOrderStatusResult = restTemplate.exchange(
                "http://"+tsOrderOtherServiceUrl+":"+tsOrderOtherServicePort+"/api/v1/orderOtherService/orderOther/status/" + orderId + "/"
                    + orderStatus,
                HttpMethod.GET,
                requestEntityModifyOrderStatusResult,
                Response.class);
