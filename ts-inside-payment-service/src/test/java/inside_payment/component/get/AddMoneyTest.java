...
public class AddMoneyTest {
     ...
     */
    @Test
    public void testAddMoneyMatchingId() throws Exception {
        // Arrange
        Money money = createSampleMoney();
        addMoneyRepository.save(money);

        // Act
        mockMvc.perform(get("/api/v1/inside_pay_service/inside_payment/{userId}/{money}", money.getId(), money.getMoney())
     ...
     */
    @Test
    public void testAddMoneyMultipleId() throws Exception {
        // Arrange
        Money money = createSampleMoney();
        addMoneyRepository.save(money);

        // Act
        mockMvc.perform(get("/api/v1/inside_pay_service/inside_payment/{userId}/{money}", money.getId(), money.getMoney(), UUID.randomUUID().toString(), "1")
                        .header(HttpHeaders.ACCEPT, "application/json"))
                .andExpect(status().isOk())
                // Assert
                .andExpect(jsonPath("$.status", is(1)))
     ...
     */
    @Test
    public void FAILING_testAddMoneyNoMatchingId() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/inside_pay_service/inside_payment/{userId}/{money}", UUID.randomUUID().toString(), "1")
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(0)))
                .andExpect(jsonPath("$.msg", is("Add Money Failed")))
                .andExpect(jsonPath("$.data", nullValue()));
    }
     ...
     */
    @Test
    public void testAddMoneyNonExistingId() {
        // Act & Assert
        assertThrows(IllegalArgumentException.class, () -> {
            mockMvc.perform(get("/api/v1/inside_pay_service/inside_payment/{userId}/{money}"));
        });
    }
     ...
     */
    @Test
    public void testAddMoneyMalformedURL() throws Exception {
        // Act
        mockMvc.perform(get("/api/v1/inside_pay_service/inside_payment/{userId}/{money}",UUID.randomUUID().toString() + "/" + UUID.randomUUID().toString(), '1')
                        .header(HttpHeaders.ACCEPT, "application/json"))
                // Assert
