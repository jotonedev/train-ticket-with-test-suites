...
public class RebookTest {
    ...
    @Test
    @Order(1)
    public void FAILING_testRebookCorrectObject() throws Exception {
        // Arrange
        String requestJson =
        ...
        mockMvc.perform(post("/api/v1/rebookservice/rebook")
                ...
                .andExpect(jsonPath("$.data.differenceMoney", is("95.0")))
     ...
     * <li><b>Parameters:</b></li> a valid request object where the new ticket price is higher than the old one. We achieve this
     * by setting the new tripId to a high speed train with the tripId beginning with "G" or "D".
    ...
    public void testRebookCantPayDifference() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/rebookservice/rebook")
                ...
                .andExpect(jsonPath("$.data.differenceMoney", is("150.0")));
    }
     ...
     *   <li>ts-config-service</li>
     *   </ul>
     * <li><b>Tests:</b></li> how the endpoint behaves when the loginId does not exist
     ...
     * <li><b>Related Issue:</b></li> <b>F4e:</b> The insidePaymentService compared an empty list with null for a non-existing
     * id, which causes the request to always succeed, which it should not.
     * </ul>
     * @throws Exception
     */
    @Test
    @Order(3)
    public void FAILING_testRebookLoginIdNonExisting() throws Exception {
        // Arrange
        String requestJson =
                "{\"loginId\":\"1\"," +
                        " \"orderId\":\"5ac7750c-a68c-49c0-a8c0-32776c067703\"," +
     ...
     *   <li>ts-config-service</li>
     *   </ul>
     * <li><b>Tests:</b></li> how the endpoint behaves when where the returned order from the order(Other)Service is too old
     * <li><b>Parameters:</b></li> a valid request object with an orderId that is too old
     * <li><b>Expected result:</b></li> status 0, msg "You can only change the ticket before the train start or within 2 hours after the train start.", no data
     * </ul>
     * @throws Exception
     */
    @Test
    @Order(9)
    public void testRebookDateMoreThan2Hours() throws Exception {
        // Arrange
        String requestJson =
                "{\"loginId\":\"4d2a46c7-71cb-4cf1-b5bb-b68406d9da6f\"," +
                        " \"orderId\":\"5ac7750c-a68c-49c0-a8c0-32776c067704\"," +
                        " \"oldTripId\":\"G9998\"," +
                        " \"tripId\":\"G1234\"," +
                        " \"seatType\":3," +
                        " \"date\":null}";
        ...
        mockMvc.perform(post("/api/v1/rebookservice/rebook")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(0)))
                .andExpect(jsonPath("$.msg", is("You can only change the ticket before the train start or within 2 hours after the train start.")))
                .andExpect(jsonPath("$.data", nullValue()));
    }
    ...
    @Test
    @Order(10)
    public void testRebookLoginIdWrongFormat() throws Exception {
        // Arrange
        String requestJson =
                "{\"loginId\":null," +
                        " \"orderId\":\"5ac7750c-a68c-49c0-a8c0-32776c067703\"," +
    ...
    @Test
    @Order(11)
    public void testRebookOrderIdWrongFormat() throws Exception {
        // Arrange
        String requestJson =
                "{\"loginId\":\"4d2a46c7-71cb-4cf1-b5bb-b68406d9da6f\"," +
                        " \"orderId\":null," +
    ...
    @Test
    @Order(12)
    public void testRebookUnavailableService() throws Exception {
        // Arrange
