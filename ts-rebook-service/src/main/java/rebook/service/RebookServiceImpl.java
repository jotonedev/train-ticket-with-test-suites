...
public class RebookServiceImpl implements RebookService
{
    ...
    private String tsInsidePaymentServiceUrl;

    @Value("${ts.inside.payment.service.port:18673}")
    private String tsInsidePaymentServicePort;
    ...
    public Response rebook(RebookInfo info, HttpHeaders httpHeaders)
    {
        ...
        if (!checkTime(order.getTravelDate(), order.getTravelTime())) {
            return new Response<>(0,
                "You can only change the ticket before the train start or within 2 hours after the train start.", null);
        }
    ...
    public Response payDifference(RebookInfo info, HttpHeaders httpHeaders)
    {
        ...
        if (payDifferentMoney(info.getOrderId(), info.getTripId(), info.getLoginId(),
            priceNew.subtract(priceOld).toString(), httpHeaders))
        {
    ...
    private Response updateOrder(Order order, RebookInfo info, TripAllDetail gtdr, String ticketPrice,
        HttpHeaders httpHeaders)
    {
    ...
    private Response<TripAllDetail> getTripAllDetailInformation(TripAllDetailInfo gtdi, String tripId,
        HttpHeaders httpHeaders)
    {
        Response<TripAllDetail> gtdr;
        String requestUrl = "";
        if (tripId.startsWith("G") || tripId.startsWith("D")) {
            requestUrl = "http://" + tsTravelServiceUrl + ":" + tsTravelServicePort + "/api/v1/travelservice/trip_detail";
            // ts-travel-service:12346/travel/getTripAllDetailInfo
        } else {
            requestUrl = "http://" + tsTravel2ServiceUrl + ":" + tsTravel2ServicePort + "/api/v1/travel2service/trip_detail";
    ...
    private Response createOrder(Order order, String tripId, HttpHeaders httpHeaders)
    {
        ...
        if (tripId.startsWith("G") || tripId.startsWith("D")) {
            ...
            requestUrl = "http://" + tsOrderServiceUrl + ":" + tsOrderServicePort + "/api/v1/orderservice/order";
        } else {
            //ts-order-other-service:12032/orderOther/create
            requestUrl = "http://" + tsOrderOtherServiceUrl + ":" + tsOrderOtherServicePort + "/api/v1/orderOtherService/orderOther";
        }
    ...
    private Response updateOrder(Order info, String tripId, HttpHeaders httpHeaders)
    {
        ...
        if (tripGD(tripId)) {
            requestOrderUtl = "http://" + tsOrderServiceUrl + ":" + tsOrderServicePort + "/api/v1/orderservice/order";
        } else {
            requestOrderUtl = "http://" + tsOrderOtherServiceUrl + ":" + tsOrderOtherServicePort + "/api/v1/orderOtherService/orderOther";
        }
    ...
    private Response deleteOrder(String orderId, String tripId, HttpHeaders httpHeaders)
    {
        ...
        String requestUrl = "";
        if (tripGD(tripId)) {
            requestUrl = "http://" + tsOrderServiceUrl + ":" + tsOrderServicePort + "/api/v1/orderservice/order/" + orderId;
        } else {
            requestUrl = "http://" + tsOrderOtherServiceUrl + ":" + tsOrderOtherServicePort + "/api/v1/orderOtherService/orderOther/" + orderId;
        }
    ...
    private Response<Order> getOrderByRebookInfo(RebookInfo info, HttpHeaders httpHeaders)
    {
        ...
        if (info.getOldTripId().startsWith("G") || info.getOldTripId().startsWith("D")) {
            requestUrl = "http://" + tsOrderServiceUrl + ":" + tsOrderServicePort + "/api/v1/orderservice/order/" + info.getOrderId();
        } else {
            requestUrl = "http://" + tsOrderOtherServiceUrl + ":" + tsOrderOtherServicePort + "/api/v1/orderOtherService/orderOther/" + info.getOrderId();
        }
    ...
    private String queryForStationName(String stationId, HttpHeaders httpHeaders)
    {
        ...
        ResponseEntity<Response> reQueryForStationName = restTemplate.exchange(
                "http://" +  tsStationServiceUrl + ":" + tsStationServicePort + "/api/v1/stationservice/stations/name/" + stationId,
            HttpMethod.GET,
        ...
        HttpHeaders httpHeaders)
    {
        ...
        ResponseEntity<Response> rePayDifferentMoney = restTemplate.exchange(
            "http://" + tsInsidePaymentServiceUrl + ":" + tsInsidePaymentServicePort + "/api/v1/inside_pay_service/inside_payment/difference",
            HttpMethod.POST,
    ...
    private boolean drawBackMoney(String userId, String money, HttpHeaders httpHeaders)
    {
        ...
        ResponseEntity<Response> reDrawBackMoney = restTemplate.exchange(
                "http://" + tsInsidePaymentServiceUrl + ":" + tsInsidePaymentServicePort + "/api/v1/inside_pay_service/inside_payment/drawback/" + userId + "/"
                + money,
            HttpMethod.GET,
