...
public class DistributeSeatTest {
     ...
     * <li><b>Expected result:</b></li> status 1, msg "Use a new seat number!", a configured ticket object
     * </ul>
     * @throws Exception
     */
    @Test
    public void testDistributeSeatCorrectObject() throws Exception {
        // Arrange
        ...
        // We loop from 3 to 8 to ensure that the seat number is not contained in the set of sold tickets, guaranteeing that the while loop will exit
        // See failing test case for when this is not the case.
        ...
        mockMvc.perform(post("/api/v1/seatservice/seats")
                ...
                .andExpect(jsonPath("$.data.startStation", is("1")))
                .andExpect(jsonPath("$.data.destStation", is("4")));
     ...
     */
    @Test
    public void testDistributeSeatMultipleObjects() throws Exception {
        // Arrange
        String requestJson =
                "[{\"travelDate\":\"2024-12-01\"," +
                " \"trainNumber\":\"G\"," +
     ...
     */
    @Test
    public void testDistributeSeatMalformedObject() throws Exception {
        // Arrange
     ...
     */
    @Test
    public void testDistributeSeatEmptyBody() throws Exception {
        // Arrange
        String requestJson = "";
     ...
     */
    @Test
    public void testDistributeSeatTrainNumberNotStartingWithGorD() throws Exception {
        // Mock responses of external services for every request this service does for the endpoint
    ...
    public void testDistributeSeatStartStationDestStationNonExisting() throws Exception {
        ...
        // Actual request to the endpoint we want to test
        String requestJson = "{\"travelDate\":\"2024-12-01\"," +
                " \"trainNumber\":\"Gwrong\"," +
                " \"startStation\":\"wrong\"," +
                " \"destStation\":\"wrong\"," +
                " \"seatType\":2}";
     ...
     * <li><b>Expected result:</b></li> status 1, msg "Use a new seat number!", a configured ticket object. The seatType will be assumed to be 3.
     * <li><b>Related Issue:</b></li> <b>F21:</b> This test can sometimes cause a NullPointerException, and sometimes it succeeds.
     * This is because when creating the ticket objects, we randomly assign seatNo values. It is possible that the seatNumbers
     * of the tickets happen to contain all seatNumbers that are within the range of {@code rand.nextInt(range) + 1}. This will caue
     * {@code while(isContained(soldTickets,seat))} to always be true, eventually crash and trigger the fallback response with null values.
     * Below this test case you find a test case that is intentionally configured to trigger the infinite while loop as well as
     * a testcase with a correct setup that should always succeed.<br>
     * This test case was executed 100 times using the {@code @RepeatedTest} annotation. It failed 21 times and succeeded 79 times.<br>
     * </ul>
     * @throws Exception
     */
    @Test
    public void UNSTABLE_testDistributeSeatSeatTypeOutOfRange() throws Exception {
        // Arrange
        // Mock responses of external services for every request this service does for the endpoint
        Route route = new Route();
        route.setStations(new ArrayList<>());
        ...
        // This is the part that can cause a NullPointerException. If the random seat numbers happen to contain all seat numbers
        // that are possible to be created with rand.nextInt(range) + 1, the while loop will never exit.
        LeftTicketInfo leftTicketInfo = new LeftTicketInfo();
        leftTicketInfo.setSoldTickets(new HashSet<>());
        for(int i = 0; i < 5; i++) {
            Ticket ticket = new Ticket();
     ...
     * <li><b>Parameters:</b></li> a valid request object with a seatType of 0
     * <li><b>Expected result:</b></li> status 1, msg "Use a new seat number!", a configured ticket object. The seatType will be assumed to be 3.
     * <li><b>Related Issue:</b></li> <b>F21:</b> This test causes a NullPointerException.
     * This is because when creating the ticket objects, we assign seatNo values that happen to contain all seatNumbers
     * that are within the range of {@code rand.nextInt(range) + 1}.
     * This will caue {@code while(isContained(soldTickets,seat))} to always be true, eventually crash and trigger
     * the fallback response with null values.
     * </ul>
     * @throws Exception
     */
    @Test
    public void FAILING_testDistributeSeatSeatTypeOutOfRange() throws Exception {
        // Arrange
        // Mock responses of external services for every request this service does for the endpoint
        Route route = new Route();
        route.setStations(new ArrayList<>());
        ...
        // Here, we intentionally configure the response so that all seat numbers are contained within the range of rand.nextInt(range) + 1
        LeftTicketInfo leftTicketInfo = new LeftTicketInfo();
        leftTicketInfo.setSoldTickets(new HashSet<>());
        for(int i = 0; i < 5; i++) {
            Ticket ticket = new Ticket();
    ...
    /**
     * <ul>
     * <li><b>Tests:</b></li> how the endpoint behaves when the seat number is already used
     * <li><b>Mocked:</b></li>
     *   <ul>
     *   <li>ts-order-service</li>
     *   <li>ts-travel-service</li>
     *   </ul>
     * <li><b>Parameters:</b></li> a valid request object. The responses of the called services are configured so that the seat number is already used.
     * <li><b>Expected result:</b></li> status 1, msg "Use the previous distributed seat number!", a configured ticket object.
     * </ul>
     * @throws Exception
     */
    @Test
    public void testDistributeSeatSeatReused() throws Exception {
        // Arrange
        // Mock responses of external services for every request this service does for the endpoint
        Route route = new Route();
        route.setStations(new ArrayList<>());
        ...
        mockMvc.perform(post("/api/v1/seatservice/seats")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status", is(1)))
                .andExpect(jsonPath("$.msg", is("Use the previous distributed seat number!")))
                .andExpect(jsonPath("$.data.startStation", is("3")))
                .andExpect(jsonPath("$.data.destStation", is("4")));
    ...
    /**
     * <ul>
     * <li><b>Tests:</b></li> how the endpoint behaves when there is no free seat for the seat request
     * <li><b>Mocked:</b></li>
     *   <ul>
     *   <li>ts-order-service</li>
     *   <li>ts-travel-service</li>
     *   </ul>
     * <li><b>Parameters:</b></li> a valid request object. The responses of the called services are configured so that there is no free seat for the seat request.
     * <li><b>Expected result:</b></li> fallback response on error: 200 OK, status message and data are null.
     * <li><b>Related Issue:</b></li> <b>S1:</b> The issue with the fallback response is that its message to the user
     * is meaningless. Therefore, it is unknown whether the developer knew of the occurrence of that specific error or not.
     * </ul>
     ...
     */
    @Test
    public void testDistributeSeatNoSeat() throws Exception {
        // Arrange
        // Mock responses of external services for every request this service does for the endpoint
        Route route = new Route();
        route.setStations(new ArrayList<>());
